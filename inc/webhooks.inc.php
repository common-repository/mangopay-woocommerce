<?php
/**
 * Incoming webhooks handling methods for MANGOPAY WooCommerce Plugin
 * 
 */
class mangopayWCWebHooks {
	
	const WEBHOOK_PREFIX 	= 'mangopay_webhooks';
	...
	public function __construct( $mangopayWCMain=NULL ) {
		...
		/** Create our own URL endpoints for processing incoming MANGOPAY webhooks **/
	...
	/**
	 * Handle incoming MANGOPAY webhooks
	 *
	 */
	private function handleRequest() {
		
		/* add sleep cause hook can be to fast and arrived before woocommerce finish is work */
		sleep(5);
		...
		/** Note: if nothing matches, it will be handled later on **/
		preg_match( '/^WC Order #(\d+)$/', $payin->Tag, $matches );
		$exit_early = false;
		if(isset($matches[1])){
			
			$order_id = $matches[1];
			
			/** Test if the order is already processing **/
			$order = new WC_Order( $order_id );
			if($order->get_status() == "processing"){
				
				/** If we are processing, it means we already handled this webhook and should not re-do it **/
				$exit_early = true;
			}
			
			/** If we already have a mp_success_transaction_id we already processed the order  **/
			$transaction_id = get_post_meta( $order_id, 'mp_success_transaction_id', true );
			if($transaction_id){
				$exit_early = true;
			}
			
			/** If one of the conditions is met, exit right away **/
			if($exit_early){
				$order->add_order_note( sprintf( 
					__( 'A redundant webhook with RessourceId %1s has been recieved and discarded', 'mangopay' ),
					$this->payload['RessourceId']	
				) );
				echo '200 (OK)';
				exit;
		...
		//$order->add_order_note( __( 'Webhook RessourceId to test:', 'mangopay' ) . ' ' . $this->payload['RessourceId'] );	//Debug
	...
	private function get_webhook_suffix() {
		...
		if( $this->mangopayWCMain->options['webhook_key'] == $endpoint_suffix )
			$endpoint_suffix = '';
	 ...
	 */
	 ...
	 private function verify_payment( $payin ) {
	 	...
	 	if( !preg_match( '/^WC Order #(\d+)$/', $payin->Tag, $matches ) ) {
	 		$this->warn_owner( sprintf(
 				__( 'MANGOPAY Payin does not contain a WooCommerce Order ID reference for Resource ID: %1$s', 'mangopay' ),
 				$this->payload['RessourceId']
	 		) );
	 		return false;
	 	}
	 	...
	 	$card_payment = false;
	 	if( get_post_meta( $order_id, 'mangopay_payment_type', true ) == 'card' )
	 		$card_payment = true;
	 		 	
	 	if( !$card_payment ) {
	 			 		
	 		/** Check that this order was paid by bankwire **/
	 		if( get_post_meta( $order_id, 'mangopay_payment_type', true ) != 'bank_wire' ) {
	 			$this->warn_owner( sprintf(
	 					__( 'MANGOPAY Payin Resource ID: %1$s references WooCommerce Order ID: %2$s which was not paid by bank wire', 'mangopay' ),
	 					$this->payload['RessourceId'],
	 					$order_id
	 			), $order_id );
	 			return false;
	 		...
	 		if( $order_payment_ref = get_post_meta( $order_id, 'mangopay_payment_ref', true ) )
		 	...
		 	if( !in_array( $payin->Id, $transaction_ids ) ) {
		 		...
		 		$this->warn_owner( sprintf(
	 				__( 'MANGOPAY Payin Resource ID: %1$s references wrong WooCommerce Order ID: %2$s', 'mangopay' ),
	 				$this->payload['RessourceId'],
	 				$order_id
		 		), $order_id );
		 		return false;
	 	...
	 	if( 'SUCCEEDED' != $payin->Status ) {
	 		...
	 		$message = __( 'MANGOPAY Payin did not succeed for Resource ID: %1$s concerning WooCommerce Order ID: %2$s', 'mangopay' );
	 		...
	 		if( $card_payment ) {
	 			...
	 			if( 
	 				...
	 				$order->add_order_note( __( 'Hook note: ', 'mangopay' ) . $payin->ResultMessage );
	 	...
	 	if( !$card_payment && $payin->DebitedFunds != $payin->PaymentDetails->DeclaredDebitedFunds ) {
	 		$this->warn_owner( sprintf(
 				__( 'MANGOPAY Payin debited funds do not match declared for Resource ID: %1$s', 'mangopay' ),
 				$this->payload['RessourceId'],
 				$order_id
	 		), $order_id );
	 		return false;
	 	...
	 	if( !$card_payment && $payin->Fees != $payin->PaymentDetails->DeclaredFees ) {
	 		$this->warn_owner( sprintf(
 				__( 'MANGOPAY Payin fees do not match declared for Resource ID: %1$s', 'mangopay' ),
 				$this->payload['RessourceId'],
 				$order_id
	 		), $order_id );
	 		return false;
	 ...
	 private function validate_order( $order_id ) {
	 	...
	 	$note = sprintf( __( 'Received valid MANGOPAY %1$s webhook', 'mangopay' ), $this->event_type );
	 	//$order		= wc_get_order( $order_id );
		$order = new WC_Order($order_id);
        ...
        if( !$this->mp->is_production() ){
            $order->add_order_note( __( 'This order was done using the Sandbox testing environment.', 'mangopay' ) );
        }
        
	 	$order->add_order_note( __( 'Hook note: ', 'mangopay' ) . $note );
	 	//$order->update_status( $order->get_status(), "CECI EST LE SUPPLEMENTAIRE?" );	 	
	 	...
	 	try{
	 		...
	 		$this->warn_owner( sprintf(
 				__( 'MANGOPAY Payin webhook could not complete payment for order: %1$s. %2$s', 'mangopay' ),
 				$order_id,
 				$error_message
	 		), $order_id );
	 	}
	 ...
	 private function check_webhook_key() {
	 	...
	 	if( $matches[1] != $this->mangopayWCMain->options['webhook_key'] )
	 		return false;
	 ...
	 private function warn_owner( $message, $order_id=NULL ) {
	 	...
	 	if( $order_id ) {
	 		...
	 		if( $order = wc_get_order( $order_id ) ) {
		 		...
		 		if( self::WARN_VENDORS ) {	// usually turned off
		 		...
		 		$order->add_order_note( __( 'Hook note: ', 'mangopay' ) . $message );
	 		}
	 	}
	 	...
	 	return
		 	wp_mail(
		 		$recipients,
		 		__( 'MANGOPAY Bank Wire webhook warning', 'mangopay' ),
		 		$message
		 	);
	 }
